...
class CertificateController {
	...
	async add(req, res) {
		try {
			const certificate = await this.certificateService.add({ body: req.body, files: req.files })
	...
	async findOne(req, res) {
		try {
			const certificate = await this.certificateService.findOne({ id: req.params.id, query: req.query })
			const response = this.handleResponseHandler.buildResponse(certificate)
	...
	async list(req, res) {
		try {
			const certificate_list = await this.certificateService.list({ query: req.query })
	...
	async update(req, res) {
		try {
			const certificate = await this.certificateService.update({ body: req.body, id: req.params.id, files: req.files })
			const response = this.handleResponseHandler.buildResponse(certificate)
	...
	async replace(req, res) {
		try {
			const certificate = await this.certificateService.replace({ body: req.body, id: req.params.id, files: req.files })
			const response = this.handleResponseHandler.buildResponse(certificate)
	...
	async remove(req, res) {
		try {
			const certificate = await this.certificateService.remove({ id: req.params.id })
			const response = this.handleResponseHandler.buildResponse(certificate)
