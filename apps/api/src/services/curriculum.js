...
const CurriculumInterfaces = require('../interfaces/curriculum')
const CurriculumContract = require('../contracts/curriculum')
...
class CurriculumService {
	...
	constructor() {
		...
		this.curriculumInterface = CurriculumInterfaces.getInstance()
		this.curriculumContract = CurriculumContract.getInstance()
	...
	static getInstance() {
		if (!this.instance) this.instance = new CurriculumService()
	...
	async add(config = {}) {
		...
		const data = Array.isArray(unflattenedBody)
			? unflattenedBody.map(el => this.curriculumInterface.getCreateInterface().parse(el))
			: this.curriculumInterface.getCreateInterface().parse(unflattenedBody)
		
		let curriculum = await this.repository.add('curriculum', { data, options })

		// 2. Handle files if present
		if (files && files.length) {
			const curriculumId = Array.isArray(curriculum) ? curriculum[0]._id : curriculum._id
			...
			try {
				for (const file of files) {
					...
					const newFilename = `curriculum-${crypto.randomUUID()}-${originalName}`
				...
				if (Object.keys(updates).length > 0) {
					curriculum = await this.repository.update('curriculum', { id: curriculumId, data: updates, options })
				}
			} catch (err) {
		...
		return this.applayContract(curriculum)
	}
	...
	async findOne(config = {}) {
		...
		let virtuals = {}
		let relations = {}
		const query = this.curriculumInterface.getQueryInterface().parse({ _id: id, ...config.query?.query })
		if (config.query?.virtuals) virtuals = this.curriculumInterface.getVirtualsInterface().parse(config.query.virtuals)
		if (config.query?.relations) relations = this.curriculumInterface.getRelationsInterface().parse(config.query.relations)
		const options = config.options || {}

		const result = await this.repository.list('curriculum', { query, virtuals, relations, options })
	...
	async list(config = {}) {
		...
		let virtuals = {}
		let relations = {}
		if (config.query?.query) query = this.curriculumInterface.getQueryInterface().parse(config.query.query)
		if (config.query?.virtuals) virtuals = this.curriculumInterface.getVirtualsInterface().parse(config.query.virtuals)
		if (config.query?.relations) relations = this.curriculumInterface.getRelationsInterface().parse(config.query.relations)
		...
		const curriculum_list = await this.repository.list('curriculum', { query, virtuals, relations, size, page, sort, options })
		curriculum_list.records = this.applayContract(curriculum_list.records)
	...
	async update(config = {}) {
		...
		const existingCurriculum = await this.findOne({ id })
		const destinationPath = envVariables.API_UPLOAD_PATH || path.join(process.cwd(), 'api-uploads')

		// 1. Initial update with JSON data
		const payload = this.curriculumInterface.getUpdateInterface().parse(data)
		let curriculum = await this.repository.update('curriculum', { id, data: payload, options })
		
		const existingObj = existingCurriculum.toObject ? existingCurriculum.toObject() : existingCurriculum;
		...
		if (files && files.length) {
			...
			try {
				...
				if (Object.keys(updates).length > 0) {
					curriculum = await await this.repository.update('curriculum', { id, data: updates, options })
				}
			} catch (err) {
				for (const url of savedFiles) {
					await this.storageProvider.deleteFile(url, destinationPath).catch(() => {})
	...
	async replace(config = {}) {
		...
		const payload = this.curriculumInterface.getUpdateInterface().parse(data)
		let curriculum = await await this.repository.replace('curriculum', { id, data: payload, options })
		...
		if (files && files.length) {
			...
			try {
				...
				if (Object.keys(updates).length > 0) {
					curriculum = await this.repository.update('curriculum', { id, data: updates, options })
				}
			} catch (err) {
				for (const url of savedFiles) {
					await this.storageProvider.deleteFile(url, destinationPath).catch(() => {})
	...
	async remove(config = {}) {
		...
		const repositoryResponse = await await this.repository.remove('curriculum', { id, options })
	...
	applayContract(payload) {
		const contract = this.curriculumContract.getContract()
