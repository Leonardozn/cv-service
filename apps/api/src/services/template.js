...
const TemplateInterfaces = require('../interfaces/template')
const TemplateContract = require('../contracts/template')
...
class TemplateService {
	...
	constructor() {
		...
		this.templateInterface = TemplateInterfaces.getInstance()
		this.templateContract = TemplateContract.getInstance()
	...
	async add(config = {}) {
		...
		const data = Array.isArray(unflattenedBody)
			? unflattenedBody.map(el => this.templateInterface.getCreateInterface().parse(el))
			: this.templateInterface.getCreateInterface().parse(unflattenedBody)
		
		let template = await this.repository.add('template', { data, options })

		// 2. Handle files if present
		if (files && files.length) {
			const templateId = Array.isArray(template) ? template[0]._id : template._id
			...
			try {
				for (const file of files) {
					...
					const newFilename = `template-${crypto.randomUUID()}-${originalName}`
				...
				if (Object.keys(updates).length > 0) {
					template = await this.repository.update('template', { id: templateId, data: updates, options })
				}
			} catch (err) {
		...
		return this.applayContract(template)
	}
	...
	async findOne(config = {}) {
		...
		let virtuals = {}
		let relations = {}
		const query = this.templateInterface.getQueryInterface().parse({ _id: id, ...config.query?.query })
		if (config.query?.virtuals) virtuals = this.templateInterface.getVirtualsInterface().parse(config.query.virtuals)
		if (config.query?.relations) relations = this.templateInterface.getRelationsInterface().parse(config.query.relations)
		const options = config.options || {}

		const result = await this.repository.list('template', { query, virtuals, relations, options })
	...
	async list(config = {}) {
		...
		let virtuals = {}
		let relations = {}
		if (config.query?.query) query = this.templateInterface.getQueryInterface().parse(config.query.query)
		if (config.query?.virtuals) virtuals = this.templateInterface.getVirtualsInterface().parse(config.query.virtuals)
		if (config.query?.relations) relations = this.templateInterface.getRelationsInterface().parse(config.query.relations)
		...
		const template_list = await this.repository.list('template', { query, virtuals, relations, size, page, sort, options })
		template_list.records = this.applayContract(template_list.records)
	...
	async update(config = {}) {
		...
		const payload = this.templateInterface.getUpdateInterface().parse(data)
		let template = await this.repository.update('template', { id, data: payload, options })
		
		const existingObj = existingTemplate.toObject ? existingTemplate.toObject() : existingTemplate;
		...
		if (files && files.length) {
			...
			try {
				...
				if (Object.keys(updates).length > 0) {
					template = await await this.repository.update('template', { id, data: updates, options })
				}
			} catch (err) {
				for (const url of savedFiles) {
					await this.storageProvider.deleteFile(url, destinationPath).catch(() => {})
	...
	async replace(config = {}) {
		...
		const payload = this.templateInterface.getUpdateInterface().parse(data)
		let template = await await this.repository.replace('template', { id, data: payload, options })
		...
		if (files && files.length) {
			...
			try {
				...
				if (Object.keys(updates).length > 0) {
					template = await this.repository.update('template', { id, data: updates, options })
				}
			} catch (err) {
				for (const url of savedFiles) {
					await this.storageProvider.deleteFile(url, destinationPath).catch(() => {})
	...
	async remove(config = {}) {
		...
		const repositoryResponse = await await this.repository.remove('template', { id, options })
	...
	applayContract(payload) {
		const contract = this.templateContract.getContract()
